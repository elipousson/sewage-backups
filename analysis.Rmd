...
We also also matched each tract to a wastewater treatment plant by checking whether tracts were located inside or outside of the service area for the Back River Wastewater Treatment Plant. We traced the boundaries of the service area using a 2004 map (published in the City of Baltimore 2006 Comprehensive Water and Wastewater Treatment Plan) that we previously [georectified using the Map Warper web service](http://mapwarper.net/maps/46308) [@CityBaltimoreComprehensive2006]. For any tracts that crossed the boundary between the Back River and Patapsco service area, we matched the tract to the service area that contained a majority of the area of the tract.
...
# Label combined summary of requests/ACS data
acs_summary_requests_tracts$quartile_median_household_income <- as.factor(acs_summary_requests_tracts$quartile_median_household_income)

var_label(acs_summary_requests_tracts) <- rio::import("data/acs_summary_requests_tracts_labels.csv", "csv") %>%
  select(variable, label) %>%
  codebook::dict_to_list()
...
# Label combined summary of requests/ACS data
acs_summary_requests_tracts_annual$quartile_median_household_income <- as.factor(acs_summary_requests_tracts_annual$quartile_median_household_income)

var_label(acs_summary_requests_tracts_annual) <- rio::import("data/acs_summary_requests_tracts_labels.csv", "csv") %>%
  select(variable, label) %>%
  codebook::dict_to_list()
...
# acs_requests_tracts <- requests_tracts %>% full_join(acs_tract_wide, by = "geoid") %>% write_csv('acs_requests_tracts.csv')
```
...
```{r request_count_by_year_facet_map, fig.fullwidth=TRUE}
...
## Requests by treatment service area
...
```{r glm_nb}
fit_glmnb <- function(df) {
  MASS::glm.nb(request_count ~ pct_black_pop + quartile_median_household_income + pct_owner_occupied + pct_built_1949earlier + offset(log(total_pop)), data = df)
}

model_selected <- acs_summary_requests_tracts %>% 
  fit_glmnb()

model_selected_annual <- acs_summary_requests_tracts_annual %>%
  filter(!is.na(year_created), year_created < 2020) %>%
  group_by(year_created) %>%
  nest() %>%
  mutate(
    model = map(data, fit_glmnb),
  )
```

```{r glm_nb_table, results = "show"}
# Create a table for the model with all data
model_selected_tbl <- model_selected %>% 
  tbl_regression(exponentiate = TRUE)

# Fix value labels for median income quartiles
model_selected_tbl[["table_body"]][["label"]][3:6] <- c("$19,375 to $48,092 (1)", "$49,054 to $62,250 (2)", "$62,695 to $82,139 (3)", "$83,929 to $213,200 (4)")

# Display table of the combined model
model_selected_tbl
```
...
```{r glm_nb_table_each_year, results = "show"}
# Create tables models with each year of data
model_selected_2017 <- model_selected_annual[[3]][[1]] %>% 
  tbl_regression(exponentiate = TRUE)

model_selected_2018 <- model_selected_annual[[3]][[2]] %>% 
  tbl_regression(exponentiate = TRUE)

model_selected_2019 <- model_selected_annual[[3]][[3]] %>%
  tbl_regression(exponentiate = TRUE)

# Merge tables
model_selected_each_year <- tbl_merge(list(model_selected_2017, model_selected_2018, model_selected_2019), tab_spanner = c("2017", "2018", "2019"))

# Fix quartile labels
model_selected_each_year[["table_body"]][["label"]][2:6] <- c("Median household income (quartile)", "$19,375 to $48,092 (1)", "$49,054 to $62,250 (2)", "$62,695 to $82,139 (3)", "$83,929 to $213,200 (4)")

model_selected_each_year
```
